...
class SEGNN(nn.Module):
    ...
    def __init__(self, input_irreps, hidden_irreps, output_irreps, edge_attr_irreps, node_attr_irreps, num_layers, norm=None, pool='avg', task='graph', init="kaiming_uniform", additional_message_irreps=None):
        super().__init__()
        self.task = task
        # Create network, embedding first
        self.embedding_layer_1 = O3TensorProductSwishGate(input_irreps, hidden_irreps, node_attr_irreps, init=init)
        self.embedding_layer_2 = O3TensorProduct(hidden_irreps, hidden_irreps, node_attr_irreps, init=init)

        # Message passing layers.
        layers = []
        for i in range(num_layers):
            layers.append(SEGNNLayer(hidden_irreps, hidden_irreps, hidden_irreps, edge_attr_irreps,
...
class SEGNNLayer(MessagePassing):
    ...
    def __init__(self, input_irreps, hidden_irreps, output_irreps, edge_attr_irreps, node_attr_irreps, norm=None, init="kaiming_uniform", additional_message_irreps=None):
        ...
        self.message_layer_2 = O3TensorProductSwishGate(hidden_irreps, hidden_irreps, edge_attr_irreps, init=init)
        self.update_layer_1 = O3TensorProductSwishGate(update_input_irreps, hidden_irreps, node_attr_irreps, init=init)
        self.update_layer_2 = O3TensorProduct(hidden_irreps, hidden_irreps, node_attr_irreps, init=init)
